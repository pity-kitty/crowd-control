...
public class AnimationGenerator : EditorWindow
{
    ...
    class AnimationBakeInfo
    {
        ...
        public AnimatorClipInfoEx clipInfoEx;
    ...
    void GenerateAnimation()
    {
        if (generateInfo.Count > 0 && workingInfo == null)
        {
            ...
            workingInfo.animator.Play(workingInfo.clipInfoEx.animHashName);
            workingInfo.animator.Update(0);
        ...
        if (workingInfo != null)
        {
            ...
            for (int j = 0; j != workingInfo.meshRender.Length; ++j)
            {
                GenerateBoneMatrix(workingInfo.meshRender[j].name.GetHashCode(),
                                        workingInfo.clipInfoEx.animHashName,
                                        workingInfo.workingFrame,
                                        Matrix4x4.identity,
                                        false);
            }
            //Debug.Log("The length is" + workingInfo.animator.velocity.magnitude);
            workingInfo.clipInfoEx.velocity[workingInfo.workingFrame] = workingInfo.animator.velocity;
            workingInfo.clipInfoEx.angularVelocity[workingInfo.workingFrame] = workingInfo.animator.angularVelocity * Mathf.Rad2Deg;

            //add the workingInfo when workingFrame more than totalFrame
            if (++workingInfo.workingFrame >= workingInfo.clipInfoEx.totalFrame)
            {
                aniInfo.Add(workingInfo.clipInfoEx);
            ...
            float deltaTime = workingInfo.length / (workingInfo.clipInfoEx.totalFrame - 1);
            workingInfo.animator.Update(deltaTime);
            EditorUtility.DisplayProgressBar("Generating Animations",
                string.Format("Animation '{0}' is Generating.", workingInfo.clipInfoEx.animHashName),
                ((float)(generateCount - generateInfo.Count) / generateCount));

            Debug.Log(string.Format("Animation '{0}' is Generating. Current frame is {1}", workingInfo.clipInfoEx.animHashName, workingInfo.workingFrame));
        }
    }
    ...
    void BakeAnimation()
    {
        ...
        if (generatedPrefab != null)
        {
            ...
            foreach (AnimationClip animClip in clips)
            {
                ...
                AnimatorClipInfoEx clipInfoEx = new AnimatorClipInfoEx();
                clipInfoEx.animationNameHash = aniName;
                clipInfoEx.animationIndex = frames;
                clipInfoEx.totalFrame = bakeFrames;
                ...
                for (int i = 0; i < bakeFrames; i += 1)
                {
                    ...
                    for (int j = 0; j != meshRender.Length; ++j)
                    {
                        GenerateBoneMatrix(meshRender[j].name.GetHashCode(),
                                                ...
                                                i,
                                                rootMatrix1stFrame,
                                                clipInfoEx.rootMotion);
                    }
                }

                aniInfo.Add(clipInfoEx);
                frames += bakeFrames;
                SetupAnimationTexture(aniInfo);
            }
        ...
        int animationIndex)
    {
        ...
        for (int i = 0; i != stateMachine.states.Length; ++i)
        {
            ...
            {
                if (obj.clipInfoEx.animStrName == clip.name)
                {
                    needBake = false;
                    break;
                }
            ...
            bake.layer = layer;
            bake.clipInfoEx = new AnimatorClipInfoEx();
            bake.clipInfoEx.animStrName = clip.name;
            bake.clipInfoEx.animHashName = state.state.nameHash;
            bake.clipInfoEx.animationIndex = animationIndex;
            bake.clipInfoEx.totalFrame = (int)(bake.length * bakeFPS + 0.5f) + 1;
            bake.clipInfoEx.totalFrame = Mathf.Clamp(bake.clipInfoEx.totalFrame, 1, bake.clipInfoEx.totalFrame);
            bake.clipInfoEx.fps = bakeFPS;
            bake.clipInfoEx.rootMotion = true;
            bake.clipInfoEx.wrapMode = clip.isLooping ? WrapMode.Loop : clip.wrapMode;
            if (bake.clipInfoEx.rootMotion)
            {
                bake.clipInfoEx.velocity = new Vector3[bake.clipInfoEx.totalFrame];
                bake.clipInfoEx.angularVelocity = new Vector3[bake.clipInfoEx.totalFrame];
            }
            generateInfo.Add(bake);
            animationIndex += bake.clipInfoEx.totalFrame;
            totalFrame += bake.clipInfoEx.totalFrame;

            bake.clipInfoEx.eventList = new List<AnimationEvent>();
            //AnimationClip clip = state.state.motion as AnimationClip;
            foreach (var evt in clip.events)
            {
                ...
                bake.clipInfoEx.eventList.Add(aniEvent);
            }
    ...
    private void SaveAnimationInfo(string name)
    {
        ...
        Debug.Log("AniInfo Count is "+aniInfo.Count);
        foreach (var obj in aniInfo)
        {
            AnimatorClipInfoEx clipInfoEx = (AnimatorClipInfoEx)obj;
            //writer.Write(clipInfoEx.animationNameHash);
            writer.Write(clipInfoEx.animStrName);
            writer.Write(clipInfoEx.animationIndex);
            writer.Write(clipInfoEx.textureIndex);
            writer.Write(clipInfoEx.totalFrame);
            writer.Write(clipInfoEx.fps);
            writer.Write(clipInfoEx.rootMotion);
            writer.Write((int)clipInfoEx.wrapMode);
          
            if (clipInfoEx.rootMotion)
            {
                Debug.Assert(clipInfoEx.totalFrame == clipInfoEx.velocity.Length);
                for (int i = 0; i != clipInfoEx.velocity.Length; ++i)
                {
                    writer.Write(clipInfoEx.velocity[i].x);
                    writer.Write(clipInfoEx.velocity[i].y);
                    writer.Write(clipInfoEx.velocity[i].z);

                    writer.Write(clipInfoEx.angularVelocity[i].x);
                    writer.Write(clipInfoEx.angularVelocity[i].y);
                    writer.Write(clipInfoEx.angularVelocity[i].z);
                }
            }
            writer.Write(clipInfoEx.eventList.Count);
            foreach (var evt in clipInfoEx.eventList)
            {
        ...
        bool rootMotion)
    {
        ...
        //we should ensure the vertexCache be non null
    ...
    private void PrepareBoneTexture(ArrayList infoList)
    {
        ...
        {
            AnimatorClipInfoEx clipInfoEx = infoList[i] as AnimatorClipInfoEx;
            frames[i] = clipInfoEx.totalFrame;
        }
    ...
    public void SetupAnimationTexture(ArrayList infoList)
    {
        ...
        for (int i = 0; i != currentDataIndex; ++i)
        {
            ...
            if (preNameCode != matrixData.stateName)
            {
                ...
                {
                    AnimatorClipInfoEx clipInfoEx = obj as AnimatorClipInfoEx;
                    if (clipInfoEx.animationNameHash == matrixData.stateName)
                    {
                        clipInfoEx.animationIndex = pixelx / textureBlockWidth + pixely / textureBlockHeight * bakedBoneTexture[bakedTextureIndex].width / textureBlockWidth;
                        clipInfoEx.textureIndex = bakedTextureIndex;
                    }
                }
            }
            if (matrixData.boneMatrix != null)
